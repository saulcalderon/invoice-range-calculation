...
function calculateInvoiceDates(configuration: InvoiceConfig): Date[] {
  ...
  while (evaluatedDate < endDate) {
    ...
    const dateWithWeeks = setWeek(startDateOfWeek, weekIndex + 2);
  ...
  if (isSameDay(endDate, lastInvoice) || isAfter(endDate, lastInvoice)) {
    invoiceDates.pop();
...
const parsedEndDate = parse('2024-08-10', 'yyyy-MM-dd', new Date());
